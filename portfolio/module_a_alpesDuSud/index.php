<?php require "view/header.inc.php"; ?> 

<main id="index">

	<section>

		<article>
			<h2 class="generalTitle">ACCUEIL</h2>
			<p>En juin 2015, j'ai décidé d'effectuer à pied la traversée des Alpes du Sud, entre Monaco et Valloire, au Sud de la Savoie. Il s'agissait là de ma première grande randonnée. Elle s'est effectuée en 3 semaines sur 19 étapes, de 15 à 25 km pour la plupart.</p>
			<div class="imgCenter">
				<img src="public/images/home.JPG" alt="Face au Pelvoux" />
			</div>
		</article>

		<article>
			<h3 class="subTitle">Qui ?</h3>
			<p>J'ai fait l'intégralité de la randonnée en solo, mis à part quelques bouts d'étapes, où j'ai croisé des randonneurs qui emprutaient le même chemin que moi.</p>
		</article>

		<article>			
			<h3 class="subTitle">Où ?</h3>
			<p>Pour rallier Monaco au début de la Savoie, je suis passé par différents massifs :</p>
			<ul>
				<li>l'Arrière-Pays Niçois,</li>
				<li>le Mercantour,</li>
				<li>le Chambeyron,</li>
				<li>le Queyras,</li>
				<li>les Ecrins,</li>
				<li>et les Cerces.</li>
			</ul>
		</article>

		<article>
			<h3 class="subTitle">Quand ?</h3>
			<p>Le trek s'est déroulé du 1<sup>er</sup> au 21 juin 2015. j'ai choisi cette période pour 2 raisons. Tout d'abord, les températures en montagne sont généralement, à cette période, suffisamment élevées pour randonner, sans atteindre la canicule. Ensuite, il y a encore peu de marcheurs sur les sentiers début juin, ce qui permet de bénéficier de plus de tranquilité.</p>
		</article>

		<article>
			<h3 class="subTitle">Comment ?</h3>
			<p>A pied, évidemment !</p>
			<p>Le but étant d'effectuer l'intégralité de la randonnée uniquement  en marchant, sans utiliser de transport mécanique.</p>
		</article>

		<article>
			<h3 class="subTitle">Pourquoi ?</h3>
			<p>Tout a commencé à l'été 2014. J'ai découvert par hasard le blog d'un randonneur qui effectue tous les ans de longues marches à travers tous les continents. Des aventures pouvant aller de 10 jours jusqu'à 10 mois.</p>
			<p>Un an après, j'ai décidé de me lancer, à mon tour, dans ma première grande randonnée.</p>
		</article>

	</section><!--

	--><aside>

		<h3 class="subTitle">LES ALPES DU SUD</h3>
		<p>Les Alpes du Sud sont une zone géographique naturelle des Alpes Françaises, située principalement dans la région Provence-Alpes-Côte-d'Azur, le reste se trouvant dans le département de la Drôme.</p>

		<div class="imgCenter">
			<img src="public/images/southernAlps.JPG" alt="Auron" />
		</div>

		<p>La ligne de crêtes séparant les Alpes du Sud de celles du Nord suit la limite Sud du Vercors, le Col de la Croix-Haute, les crêtes au Sud du Dévoluy, le Col Bayard, la limite sud des Ecrins, les Cols du Lautaret et du Galibier, et le fond de la vallée de la Clarée.</p>

		<p>Les massifs les plus connus des Alpes du Sud sont :</p>
		<ul>
			<li>l'Arrière-Pays Niçois,</li>
			<li>le Mercantour,</li>
			<li>le Queyras,</li>
			<li>les Ecrins,</li>
			<li>le Lubéron,</li>
			<li>les Monts de Vaucluse, où se situe le Mont Ventoux,</li>
			<li>et le Vercors, en partie seulement inclus dans les Alpes du Sud.</li>
		</ul>

		<a href="https://fr.wikipedia.org/wiki/Alpes_du_Sud_(France)" target="new"  class="proportionalFontSize"><small>Source : Wikipedia</small></a>
		
	</aside>

</main>	

<?php require "view/footer.inc.php"; ?>